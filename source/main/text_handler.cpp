...
/*const unsigned char knumbers_in_bytes_array[][7] = {
    {0x1F, 0x11, 0x15, 0x11, 0x1F, 0x00, '0'}, // 0 27
    {0x10, 0x12, 0x1F, 0x10, 0x10, 0x00, '1'}, // 1 28
    {0x1D, 0x15, 0x15, 0x15, 0x17, 0x00, '2'}, // 2 29
    {0x15, 0x15, 0x15, 0x15, 0x1F, 0x00, '3'}, // 3 30
...
/*
void TextPusherNumberHandler(uint8_t message[])
{
  Serial.println("Array recieved.");
    for (uint8_t i = 0; i < 4; i++)
    {
        unsigned char data = (char)message[i];
        for (uint8_t j = 0; j < 10; j++)
        {
          Serial.println(data);
          Serial.println(message[i]);
          Serial.println((char)message[i]);
            if (data == knumbers_in_bytes_array[j][6])
            {
                Serial.println(knumbers_in_bytes_array[j][6]);
                PushToScreen(knumbers_in_bytes_array[j]);
                break;
            }
        }
    }
...
/*  for (uint8_t i = 0; i < 4; i++)
  {
    unsigned char data = message[i];
    TextPusherArray(data);
  }
  //TextPusherArray(*message, 4);
}*/


/* This will stay for another round as it is more perfomant and my previous code was profiling incorrectly.
The difference is minimum but the array does show signs of multiple disadvantages.
